		...
		if ($this->RecSaved) {
			if ($this->RecNum < $this->RecNbr) {
				...
				if ($this->OnDataOk) {
					...
					if ($this->SubType !== 2)
						$this->RecSet[$this->RecKey] = $this->CurrRec; // save modifications because array reading is done without reference :(
				}
			} else {
				unset($this->CurrRec);
				$this->CurrRec = false;
...
class clsTinyButStrong
{
	...
	public function MergeField($NameLst, $Value = 'assigned', $IsUserFct = false, $DefaultPrm = false)
	{
		...
		if ($PlugIn = isset($this->_piOnMergeField))
			$ArgPi = array('', '', &$Value, 0, &$this->Source, 0, 0);
		$SubStart = 0;
		$Ok = true;
	...
	function meth_Locator_SectionAddGrp(&$LocR, $BlockName, &$BDef, $Type, $Field, $Prm)
	{
		...
		if (strpos($Field, $this->_ChrOpen) === false)
			$Field = $this->_ChrOpen . $BlockName . '.' . $Field . ';tbstype=' . $Prm . $this->_ChrClose; // tbstype is an internal parameter for catching errors
...
// This position can be useful because we don't know in advance how $Value will be replaced.
		...
		if ($Loc->Ope) {
			foreach ($Loc->OpeAct as $i => $ope) {
				switch ($ope) {
					...
					case 3:
						if (strlen('' . $CurrVal) > $Loc->OpePrm[$i])
							$CurrVal = (($Loc->OpeUtf8) ? mb_substr('' . $CurrVal, 0, $Loc->OpePrm[$i], 'UTF-8') : substr('' . $CurrVal, 0, $Loc->OpePrm[$i])) . $Loc->OpeEnd;
						break;
					case 5:
		...
		if ($Loc->ConvMode === 1) { // Usual string conversion
			if (!is_string($CurrVal))
				$CurrVal = @(string) $CurrVal; // (string) is faster than strval() and settype()
			if ($Loc->ConvStr)
				$this->meth_Conv_Str($CurrVal, $Loc->ConvBr);
...
// Mode = 3 : GetBlockSource => save $Loc->BlockSrc with TBS Def tags
	...
	function meth_Merge_AutoOn(&$Txt, $Name, $TplVar, $MergeVar)
	{
		...
		if ($MergeVar)
			$this->meth_Merge_AutoVar($this->Source, true, $Name); // merge other fields (must have subnames)
		...
		if ($Status === 0) {
			...
			if ($SubName) {
				...
				if ($x === 'if') {
					...
					$Loc->PrmLst[$x] = $v;
					if ($WithPos)
						$Loc->PrmPos[$x] = array($PosName, $PosNend, $PosVal, $Pos, $DelimChr, $DelimCnt);
				}
			}
		}
		...
		while ($TagStr !== '') {
			...
			do { // Check parentheses, relative position and single tag
				...
				if (($e > 1) and ($t[0] === '(') and ($t[$e] === ')')) {
					...
					if (($e >= 0) and ($t[$e] === '/'))
						$t = substr($t, 0, $e); // for compatibilty
					$e = false;
				}
			} while ($e !== false);
			if (isset($AliasLst[$t])) {
				...
				if (is_string($a)) {
					if ($i > 999)
						return false; // prevent from circular alias
	...
	static function f_Xml_AttFind(&$Txt, &$Loc, $Move = false, $AttDelim = false)
	{
		...
		if (isset($LocO->PrmLst[$AttLC])) {
			...
			$p[3]--;
			while ($Txt[$p[3]] === ' ')
				$p[3]--; // external end of the attribute, may has an extra spaces
			$Loc->AttEnd = $p[3];
			$Loc->AttDelimCnt = $p[5];
			$Loc->AttDelimChr = $p[4];
		...
		do {
			...
			if (($x === "\r") or ($x === "\n")) {
				...
				if ($IsRef or ($p0 != $PosBeg))
					return $p0 + 1; // Backwars => return pos without newline char. Ignore newline if it is the very first char of the search.
			}
			$p += $Inc;
		} while (true);
